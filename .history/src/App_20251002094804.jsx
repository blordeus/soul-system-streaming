...
function PlayerUI({ ALBUM, BRAND, embed=false }) {
  ...
  {/* Header */}
  <header
  className="flex items-center justify-between px-8 py-4 border-b border-[var(--border)]"
  style={{
    background: "linear-gradient(90deg, var(--accent) 0%, var(--accentSecondary) 100%)",
    color: "#fff",
  }}
>
  {BRAND.logo && (
    <img
      ...
      alt={BRAND.appName}
      className="h-8 md:h-10 drop-shadow"
    />
  )}
  <h1 className="text-lg md:text-2xl font-bold tracking-wide drop-shadow">
    {BRAND.appName}
  </h1>
</header>
